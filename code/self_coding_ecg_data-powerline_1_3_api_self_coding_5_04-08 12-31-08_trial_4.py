...
# Assuming input_data is provided externally
...
def challenger(input_data, output_data, sampling_rate=500):
    ...
    mask_50hz_input = np.isclose(freqs_input, 50, atol=0.1)
    mask_50hz_output = np.isclose(freqs_output, 50, atol=0.1)
    ...
    if power_50hz_input.size > 0 and power_50hz_output.size > 0:
        # Consider all power at 50 Hz and ensure significant reduction
        if np.all(power_50hz_output.mean() < 0.1 * power_50hz_input.mean()):
            return True
        else:
            return False
    return False
