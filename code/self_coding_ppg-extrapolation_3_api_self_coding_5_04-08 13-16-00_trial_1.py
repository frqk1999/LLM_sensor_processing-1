...
def inspection(input_data, output_data, sampling_rate=None):
    ...
    # Check if output_data has unusual values (e.g., exceedingly high or low).
    # In this context, considering PPG data, we'll assume extreme outlier values for tests could be outside of a typical physiological range.
    if np.max(output_data) > 200 or np.min(output_data) < -200:
        return False
